...
export default {
  ...
  created() {
    console.log(this.annexUrl)
    this.getcmsColumn()
    this.getcmsColumn1()
    const that = this
    const jsArr = JSON.parse(localStorage.getItem('cmsData'))
    console.log(jsArr)
    that.id = jsArr.id
    that.author = jsArr.author
    that.title = jsArr.title
    that.value = jsArr.content
    that.hobbiess = jsArr.column.columnName
    that.hobbiesss = jsArr.column.id
    that.imgListFile.push({ name: jsArr.annexUrl, url: jsArr.annexUrl })
    that.annexUrl = jsArr.annexUrl
  },
  mounted() {
    tinymce.init({})
  },
  methods: {
    ...
    addCms() {
      const data = {
        ...
        // programa: this.hobbiess + '-' + this.hobbiesss,
        // column: { id: isNaN(this.hobbiess) ? this.hobbiesss : this.hobbiess },
